...
func TestCollectionPathIsTheKindReference(t *testing.T) {
	...
	// A record verb sits behind the reserved segment, never where an id can be.
	if got, want := recordVerbPath("tasks.substrate.reamde.dev", "task", "t9", "edges", "source"),
		"/api/v1/tasks.substrate.reamde.dev/task/t9/-/edges/source"; got != want {
		t.Errorf("recordVerbPath = %q, want %q", got, want)
	}
	if got, want := verbPath("changes"), "/api/v1/-/changes"; got != want {
		t.Errorf("verbPath = %q, want %q", got, want)
	}
