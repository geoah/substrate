...
const (
	...
	// record's operational verbs live at the record, ruling A8) the
	// trigger delivery verbs hang off it.
	coreAuthority = "core.substrate.reamde.dev"

	// kindCollection is the registry: the meta-kind is
	// core.substrate.reamde.dev/kind, self-describing the way a CRD is, and a
	// collection segment is the kind's NAME (decision 0028).
	kindCollection = "kind"

	// verbSegment is the segment reserved for verbs at every depth. Nothing
	// stored can be spelled with it, so a verb never takes an id out of a
	// collection.
	verbSegment = "-"
)

// verbPath is a repository verb: /api/v1/-/{verb}. Every endpoint that acts on
// the repository rather than on one record lives here (the changefeed, the
// vocabulary apply, the blob store, GraphQL, the catalog, the OAuth doors).
func verbPath(segments ...string) string {
	return apiPrefix + "/" + verbSegment + "/" + strings.Join(segments, "/")
}
...
// repository-local one — the same two forms the reference itself has, so a
// record's path is its stored reference value character for character
// (decision 0028). Every id segment is escaped, so a record id carrying a
// slash arrives as %2F rather than as another path segment.
...
// recordVerbPath is a verb at one record: the record's path, the reserved
// segment, then the verb. The verb segments are literal, so they are joined
// rather than escaped as ids are.
func recordVerbPath(authority, kind, id string, verb ...string) string {
	return collectionPath(authority, kind, id) + "/" + verbSegment + "/" + strings.Join(verb, "/")
}
...
func (c *client) link(ctx context.Context, authority, kind, id, rel string, to substrate.EdgeRef, props map[string]any) (*substrate.Record, error) {
	...
	var e substrate.Record
	if err := c.do(ctx, http.MethodPost, recordVerbPath(authority, kind, id, "edges", rel), nil, body, &e); err != nil {
		return nil, err
	}
	return &e, nil
}
...
func (c *client) applyVocabulary(ctx context.Context, docs []map[string]any) ([]*substrate.Record, error) {
	...
	if err := c.do(ctx, http.MethodPost, verbPath("vocabulary", "apply"), nil, body, &out); err != nil {
		return nil, err
	}
...
// repository's changelog-signing seed (hex), disclosed on this response and
// never again.
