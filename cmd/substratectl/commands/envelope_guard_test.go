...
// The last segment of `kind` is the kind's NAME (decision 0033), so a word no
// kind in the package carries is refused by name and nothing is written.
func TestApplyRefusesAKindNoPackageDeclares(t *testing.T) {
	h := newHarness(t)
	h.writeConfig()
