...
func typeRecord(name, pkg, source string, definition map[string]any) map[string]any {
	...
	properties := map[string]any{
		"name": name, "authority": authority, "package": pkgName,
...
// own authority, so `syncrun` can never resolve bare — while `person`, `task`,
// `calendarevent` and `book` each still belong to exactly one authority and must.
