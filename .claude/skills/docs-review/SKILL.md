...
envelope keys in examples, `mise run` names that exist, and an index entry for
every page. Links and anchors belong to `mise run lint:docs` (lychee, offline),
which the script calls rather than reimplements and which CI's lint job runs
too. Keep the script exiting 0.
