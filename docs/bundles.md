...
empty when the bundle is ready. `POST …/bundle/{id}/-/bind` with `{input,
record}` binds an input to a chosen record (empty `record` unbinds). The verbs are `POST …/core.substrate.reamde.dev/bundle/{id}/-/disable` and its
...
The flow itself is two endpoints. `POST /api/v1/-/oauth/start` takes the
...
`GET /api/v1/-/oauth/callback`, which is unauthenticated because that
...
(`GET …/core.substrate.reamde.dev/trait/{id}/-/records`, with `…/trait/{id}/-/implementors`
...
`GET /api/v1/-/catalog` lists every shipped bundle under a `catalog`
key, each flagged `installed` for this repository;
`GET /api/v1/-/catalog/{id}` is one entry with its closure, and an
unknown id is a 404 `not_found`.

Installing from the catalog is a thin wrapper over the ordinary apply, never a
parallel path: `POST /api/v1/-/catalog/{id}/install` applies the entry's
