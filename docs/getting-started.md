...
yet. Install the bundle that ships them from the catalog built into the
binary. A catalog id is the bundle document's own id, path-escaped in the URL,
and the tasks bundle requires the people vocabulary, so people installs first:
