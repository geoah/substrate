...
Every entry carries a SHA-256 hash chaining to the previous entry's, and a
repository can additionally sign every entry with its own Ed25519 key: an
in-place edit, a reorder or a splice breaks the chain at the first touched
seq, and `repository verify` names it. [The chain](changelog.md#the-chain)
