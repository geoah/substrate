...
| **plural** | A retired word. It named the collection segment; the segment is the kind's name now, and no path anywhere reads a plural. It survives in exactly two places, both about the DECLARATION rather than an address: `data.names.plural` is still required on a kind and still stored (retiring the key needs a dialect rung to strip it from existing rows first, because the loader admits a closed key set under `names`), and `KindInfo.plural` still carries that declared value on the wire, which is what lets `substratectl get tasks` match a typed plural against the registry before addressing `/task`. Nowhere else. |
