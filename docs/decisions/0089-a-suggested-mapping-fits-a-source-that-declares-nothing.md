...
- Delete the subject check from the door and let the loader refuse whatever
  does not fit
- Re-read "fits" against 0085: an undeclared slot fits, a declaration of the
...
2. **A declaration of the source's OWN under that word blocks** (0085's rule
...
3. **A declared `subject: true` reference fits** (0085's rule 4): the
...
the REHOMED one for an import, and a pre-0085 provider's pin names whichever of
...
- Good, because a fresh repository that installs a provider and imports a
  sample gets the projection, which is what both 0048 and 0085 promise it.
...
- Bad, because a repository whose provider predates 0085 and pins the slot at
...
mapping. `TestASuggestedMappingFitsASourceThatStillDeclaresTheSlot` is 0085's
rule 4 at the door.

## More Information

Amends [0049](0049-the-owner-of-a-mappings-target-declares-it.md) and
[0085](0085-a-mapping-synthesises-its-subject-slot.md) where the catalog is
concerned: 0085 supersedes item 3 of 0049 for the loader, and this record is
