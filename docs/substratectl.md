...
`--ambiguous` lists the
[ambiguous mapping sources](projection.md#when-a-probe-finds-several-candidates-onambiguous)
of that kind, rows a probe with several candidates left unlinked
(`--ambiguous=false` is the complement).
