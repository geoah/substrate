...
# /keys is where a deployment that mints its own credential key keeps it and
# /var/lib/substrate is the data root it mounts (see compose.yaml). Docker
# copies a directory's ownership onto a fresh named volume mounted over it,
# and a path the image lacks comes up root-owned, which is what lets the
# unprivileged user write the key it mints and create the repositories
