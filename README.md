...
mints a credential key and tells you to back it up. Register with a username
and a password: this substrate asks for no invite code and no second factor,
...
# Asks for a username and a password (and, on a substrate that reads them,
# the invite code and a TOTP code from the enrollment it prints), hands you a
# recovery key, then stores the minted token as a context in
# ~/.config/substratectl.
