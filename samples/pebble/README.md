...
triggers. The bundle requires `samples.substrate.reamde.dev/tasks`, which the assistant
...
- One `samples.substrate.reamde.dev/pebble/recording` per capture, titled by
  ...
  record.
- In agent mode, one `samples.substrate.reamde.dev/pebble/instruction` at the
...
- The `pebble-on-instruction` trigger delivers each new instruction to the
  ...
  function and creates `samples.substrate.reamde.dev/tasks/task` records through
