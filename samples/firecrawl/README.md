...
`substrate.reamde.dev/core` — so a trigger-less bundle admits exactly like a
wired one.)
...
- **`scrapepage`** (python) — validates the `url` input FIRST (an absolute
  ...
  bundle's OWN package — never another bundle's type (the web harvester's
  `page` stays the harvester's; a bundle's effects stay inside the package it
...
metadata: {id: myagents.example.com/research/researcher}
data:
  authority: myagents.example.com
  package: research
  ...
  tools:
    - function: samples.substrate.reamde.dev/firecrawl/websearch
    - function: samples.substrate.reamde.dev/firecrawl/scrapepage
...
- `bundle.yaml` — the schema closure, and the whole bundle: package + bundle +
