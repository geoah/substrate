...
func TestWatchCollectionStreamsBookmarkThenChanges(t *testing.T) {
	...
	ds.commit(substrate.Change{
		...
		RecordID: "c1", Kind: "people.substrate.geoah.me/person",
	})
	...
	ds.commit(substrate.Change{
		...
		RecordID: "m1", Kind: "messaging.substrate.geoah.me/conversationmessage",
	})
	ds.commit(substrate.Change{
		TS: time.Unix(7, 0).UTC(), Actor: substrate.ActorAPI, Op: substrate.OpPatch,
		RecordID: "c2", Kind: "people.substrate.geoah.me/person",
	})
...
func TestWatchResumesFromCursor(t *testing.T) {
	...
	for i := range 3 {
		ds.commit(substrate.Change{
			...
			RecordID: "c" + string(rune('1'+i)), Kind: "people.substrate.geoah.me/person",
		})
	}
...
func TestChangesWithoutWatchIsASinglePage(t *testing.T) {
	...
	rec := env.do(t, http.MethodGet, "/api/v1/core.substrate.reamde.dev/changes?kinds=people.substrate.geoah.me/person", tok, nil)
