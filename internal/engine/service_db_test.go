...
func TestRepositoryDatasetIsolation(t *testing.T) {
	...
	task := mustPut(t, a, owner, substrate.PutInput{Kind: "samples.substrate.reamde.dev/tasks/task", Properties: map[string]any{"name": "alpha only"}})
...
func TestSchemaMetaModelProjections(t *testing.T) {
	...
	if v, _ := vocabulary.VersionValue(pkg.Properties["version"]); v != 7 || pkg.Properties["source"] != "installed" {
...
func TestWatchSignal(t *testing.T) {
	...
	task := mustPut(t, ds, owner, substrate.PutInput{Kind: "samples.substrate.reamde.dev/tasks/task", Properties: map[string]any{"name": "watch me"}})
	select {
	case seq := <-ch:
		if seq <= 0 {
...
func TestChangesFilters(t *testing.T) {
	...
	task := mustPut(t, ds, engram, substrate.PutInput{
		Kind: "samples.substrate.reamde.dev/tasks/task", Properties: map[string]any{"name": "t", "status": "proposed"},
	})
	...
	byType, err := ds.Changes(ctx, 0, substrate.ChangeFilter{
		Kinds: []string{"samples.substrate.reamde.dev/tasks/task"}, Ops: []substrate.Op{substrate.OpPatch},
	}, 100)
	if err != nil {
		t.Fatal(err)
	}
