...
// The embedding provider row and the queue it feeds: what a provider row
// refuses, what the queue does with no provider and with the wrong vector
// width, and when a vector may be published (never from a row the write has
// since moved).
