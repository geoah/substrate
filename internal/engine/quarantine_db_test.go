...
// A stored bundle closure that no longer admits under the current
...
// (which projects a valid closure) clears the quarantine. The second half is
// the parse side: what a document that will not parse at all quarantines, and
// what the envelope refuses by name.
...
// One step EARLIER than admission: a stored closure that no longer PARSES
// under this binary must not brick the repository either. The concrete
...
// loader now refuses, and the loader reports every problem in a whole
// source's document stream at once. Without per-authority rebuilding, one
// such row fails every installed authority and the repository cannot open at
// all.
