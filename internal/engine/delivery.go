...
// --- adopting a repository written before the ledger ---

// adoptLegacyLedger is a step of the open ladder (engine.go openNew). A
// repository whose changelog was stamped below deliveryLedgerDialect holds its
// trigger bookkeeping in the tables alone, and the four tables are now fold
// tables: a rebuild would clear them and a replay would bring nothing back.
// So the first open under the ledger records what the tables hold, one
// delivery entry per live trigger, in ONE transaction before anything else
// appends: the stamp that retires this step commits with every trigger's
// entry or with none, so a failure mid-way leaves the repository unstamped
// and the next open adopts again. A read-only process appends nothing and
// leaves it to the server. Rows keyed by a callable identity rather than a
// trigger id (a pre-wave-1 cursor, triggers.go adoptableLegacyCursor) belong
// to no live trigger and are not adopted; a rebuild drops them, and a default
// trigger created after that starts at its own creation.
func (ds *dataset) adoptLegacyLedger(ctx context.Context) error {
	if !ds.adoptLedger || ds.svc.readOnly {
		return nil
	}
	...
	// The parked payloads first, outside the transaction: a legacy park holds
	// every header, the query and the body, so each goes through the parking
	// policy (webhooks.go parkedPayload), which spools a body into the blob
	// store, before any of it is written into the changelog.
	// The spool appends blob manifests, and an append stamps the dialect;
	// the stamp is what retires this step, so it is held until the adoption
	// itself commits (dataset.stampHeld).
	ds.stampHeld = true
	scrubbed := map[string][]foldFailure{}
	for _, lt := range triggers {
		failures, err := ds.scrubbedFailuresOf(ctx, lt.ID)
		if err != nil {
			ds.stampHeld = false
			return fmt.Errorf("substrate/engine: adopt the parked failures of trigger %s: %w", lt.ID, err)
		}
		scrubbed[lt.ID] = failures
	}
	ds.stampHeld = false
	err = ds.inTx(ctx, substrate.ActorSystem, true, func(t *txn) error {
		for _, lt := range triggers {
			if err := t.adoptTriggerLedger(lt.ID, scrubbed[lt.ID]); err != nil {
				return fmt.Errorf("substrate/engine: adopt the delivery state of trigger %s: %w", lt.ID, err)
			}
			if fault := ds.svc.testAdoptFault; fault != nil {
				if err := fault(lt.ID); err != nil {
					return err
				}
			}
		}
	...
	ds.adoptLedger = false
	return nil
}

// scrubbedFailuresOf reads a trigger's parked failures whole, each payload
// run through the parking policy.
func (ds *dataset) scrubbedFailuresOf(ctx context.Context, triggerID string) ([]foldFailure, error) {
	...
	_ = rows.Close()
	for i := range out {
		scrubbed, err := ds.parkedPayload(ctx, out[i].Payload)
		if err != nil {
			return nil, err
		}
		out[i].Payload = scrubbed
	}
	return out, nil
}

// adoptTriggerLedger records one trigger's rows as ledger effects on a
// delivery entry of its own: its cursor, its fire state, its parked failures
// (read and scrubbed by the caller) and its paged drains, each written back
// as the same row by the fold.
func (t *txn) adoptTriggerLedger(triggerID string, failures []foldFailure) error {
	return func() error {
		var seq int64
		err := t.row(`SELECT seq FROM trigger_cursors WHERE trigger_id = $1`, triggerID).Scan(&seq)
		switch {
		...
		var at time.Time
		err = t.row(`SELECT fired_at FROM trigger_schedule WHERE trigger_id = $1`, triggerID).Scan(&at)
		switch {
		case err == nil:
			if err := t.setScheduleTx(triggerID, at); err != nil {
				return err
			}
		case !errors.Is(err, sql.ErrNoRows):
			return err
		...
		for _, f := range failures {
			if err := t.parkTx(triggerID, f); err != nil {
				return err
			}
		}
		pages, err := t.pagesOf(triggerID)
		if err != nil {
			return err
		}
		for _, p := range pages {
			if err := t.pageTx(pagedOwner{triggerID: triggerID, kind: p.Kind, identity: p.Identity},
				p.Chain, p.Cursor, int64(p.Version), int64(p.Pages), int64(p.Effects), int64(p.Bytes), p.StartedAt); err != nil {
				return err
			}
		}
		return t.settleDelivery(triggerID)
...
// pagesOf reads a trigger's paged-drain rows whole, in the ledger's shape.
func (t *txn) pagesOf(triggerID string) ([]foldPageRow, error) {
	rows, err := t.query(`
		SELECT chain, cursor, pages, version, effects, bytes, started_at, kind, identity
		FROM paged_cursors WHERE trigger_id = $1 ORDER BY chain`, triggerID)
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		var p foldPageRow
		var cursor []byte
		if err := rows.Scan(&p.Chain, &cursor, &p.Pages, &p.Version, &p.Effects, &p.Bytes, &p.StartedAt, &p.Kind, &p.Identity); err != nil {
			return nil, err
		}
		p.Cursor = json.RawMessage(cursor)
