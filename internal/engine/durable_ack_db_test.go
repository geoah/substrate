...
func TestASealedOnlyCommitInDoubtLatchesUntilTheBootRewritesTheFile(t *testing.T) {
	...
	t.Run("delete", func(t *testing.T) {
		...
		// among them, while the record stays live: a state the OAuth
		// finalizer never leaves (it runs on tombstones), so verify names
		// exactly that dangling reference and nothing else.
		report, err := s2.VerifyRepository(ctx, "geoah")
