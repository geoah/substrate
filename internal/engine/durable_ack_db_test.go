...
func openDurabilityService(t *testing.T, opts ...Option) (*service, Repository, string, string, string) {
	...
	if _, err := s.Register(ctx, substrate.RegisterInput{
		Repository: testdb.Repository(t), Password: durabilityPassword,
...
func importCopy(t *testing.T, root, repoID string) (*service, Repository) {
	...
	s2 := svc2.(*service)
	repo2, err := s2.repositoryByID(ctx, testdb.Repository(t))
	if err != nil {
		t.Fatal(err)
	}
	return s2, repo2
...
func TestACrashBetweenTheSealedStageAndTheCommitRestoresTheOldPayload(t *testing.T) {
	...
	if report, err := s2.VerifyRepository(ctx, testdb.Repository(t)); err != nil || !report.OK {
...
func TestAReadOnlyServiceRefusesToSpendATOTPStep(t *testing.T) {
	...
	_, _, err = ro.Login(ctx, substrate.LoginInput{Repository: testdb.Repository(t), Password: durabilityPassword, TOTPCode: code, Label: "ro"})
...
func TestASealedOnlyCommitInDoubtLatchesUntilTheBootRewritesTheFile(t *testing.T) {
	...
	t.Run("update", func(t *testing.T) {
		...
		t.Cleanup(func() { _ = svc2.Close() })
		s2 := svc2.(*service)
		report, err := s2.VerifyRepository(ctx, testdb.Repository(t))
