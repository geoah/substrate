...
describe("PropertiesRail", () => {
  ...
  it("lists the records that map onto this one, each a link", () => {
    const linked: SubstrateRecord = {
      ...record,
      linkedFrom: [
        {
          ref: "providers.substrate.reamde.dev/github/user/gh-chris",
        ...
        {
          ref: "providers.substrate.reamde.dev/slack/user/U123",
          kind: "providers.substrate.reamde.dev/slack/user",
          title: "Chris R.",
          property: "person",
    ...
    // The title is the label, the kind says what it is, and the mapping is
    // named so a reader knows WHY the row is here.
    expect(text).toContain("chris")
    expect(text).toContain("Chris R.")
    expect(text).toContain("user")
    expect(text).toContain("githubuserperson")
    expect(text).toContain("slackuserperson")
    // A source kind the registry does not carry still renders, inert, rather
    ...
    expect(
      links.some((href) => href?.includes("gh-chris") || href?.includes("U123"))
    ).toBe(false)
  })

  it("routes a linked source whose kind the registry knows", () => {
    const linked: SubstrateRecord = {
      ...record,
      linkedFrom: [
        {
          ref: "samples.substrate.reamde.dev/people/person/p9",
          kind: "samples.substrate.reamde.dev/people/person",
          title: "Ada",
          property: "person",
          mapping: "samples.substrate.reamde.dev/people/contactperson",
        },
      ],
    }
    const { container } = renderRail(linked)
    const href = [...container.querySelectorAll("a")]
      ...
      .find((h) => h?.includes("p9"))
    expect(href).toBe("/data/samples.substrate.reamde.dev/people/person/p9")
  })

  it("shows no Linked from section on a record nothing maps onto", () => {
    const { container } = renderRail()
    expect(container.textContent).not.toContain("Linked from")
  })
