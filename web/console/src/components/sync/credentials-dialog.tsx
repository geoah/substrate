 ...
 * stored secret reads back as its redaction marker, never its value). */
    ...
    <RecordConfigForm
      ...
      description={
        <span className="flex flex-col gap-1.5">
          <span>
            {provider.oauth
              ? `Create an app with ${name}, register the redirect address below on it, then paste the app’s client ID and secret here. Every ${name} account you connect signs in through this one app.`
              : `Paste the token or key you created with ${name}. Every ${name} account you add uses it.`}{" "}
            A secret never shows again once saved; leave it blank to keep the
            one you saved.
          </span>
          {secrets.length > 0 && (
