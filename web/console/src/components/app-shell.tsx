...
function crumbsFor(pathname: string): Crumb[] {
  ...
  if (pathname.startsWith("/views/")) {
    const id = decodeURIComponent(
      pathname.slice("/views/".length).split("/")[0] ?? ""
    )
    return [
      { label: "Apps", to: "/apps" },
      { label: id, mono: true },
...
export function AppShell() {
  ...
  // An app or view route under 768 px owns the viewport: its own header, its
  // own bottom button, no sidebar and no console header around it.
