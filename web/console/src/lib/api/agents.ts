...
import { corePath, CORE_AUTHORITY, envelopeError, VERB } from "./http"
...
export function streamChat(opts: {
  ...
  void (async () => {
    try {
      ...
      const res = await fetch(`${corePath("agent", opts.agent)}/${VERB}/chat`, {
        method: "POST",
        headers,
        body: JSON.stringify({
          thread: opts.thread ?? "",
