 ...
 * queries refetch.
 *
 * The union always carries the apps kinds (`core/view`, `core/app`), the
 * kind collection itself, so an import or a declaration edit invalidates
 * `["registry"]` and every `PropSpec` follows, and the package collection,
 * whose rows hold the versions a view's floor is held against. Invalidation
 ...
 * bounded count. Rows are batched for a beat so an import or a sync
 * refetches once.
 *
 ...
 * single-record reads are invalidated, and so are the registry and the
 * trait implementors, because a declaration can change in that gap too. A
 * later `live` is a reconnect resuming from the last seen seq, which misses
 * nothing; the lists are invalidated once more anyway, since `staleTime`
 * alone schedules no refetch.
 *
 * The resume cursor is a seq in one history generation, and a restore of the
 * repository can retire that generation while a tab is open. The transport
 * reports it as `compacted` and returns; nothing read under the old cursor
 * can be trusted, so the same handoff runs and a fresh tail opens at the new
 * head. A terminal error frame is `stopped`: the tail is left down rather
 * than hammered, and the status store says so for a screen to show. */

import { useEffect, useSyncExternalStore } from "react"
import { useQueryClient, type QueryClient } from "@tanstack/react-query"

import { APP_KIND, PACKAGE_KIND, VIEW_KIND } from "@/lib/api/apps"
import {
  watchChanges,
  type WatchHandle,
  type WatchStatus,
...
const ALWAYS = [VIEW_KIND, APP_KIND, KIND_KIND, PACKAGE_KIND]
const BATCH_MS = 150
 ...
 * handoff, which cannot know what moved). A declaration's change is also
 * the registry and the trait implementors, both read off the kinds. */
function invalidateKind(client: QueryClient, kind: string, ids?: Set<string>) {
  if (kind === KIND_KIND) {
    void client.invalidateQueries({ queryKey: ["registry"] })
    void client.invalidateQueries({ queryKey: ["trait", "implementors"] })
  }
