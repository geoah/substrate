 ...
 *
 * vitest runs a setup file for EVERY suite, and the default environment is
 * `node`, where `Element` and `Range` are not defined at all. So the whole
 * install is behind one check for a window, and a pure-logic suite skips it. */

if (typeof document !== "undefined") installBrowserGaps()
