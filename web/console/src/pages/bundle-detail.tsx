...
function SetupItemRow({ item, types }: { item: SetupItem; types: KindInfo[] }) {
  ...
  const collection = item.code === "provider" ? "llmprovider" : kind?.name
  return (
    <div className="flex items-center justify-between gap-3 rounded-md border border-warning/40 px-4 py-2.5">
      ...
      {item.record && authority && collection && (
        <Link
          to="/data/$authority/$collection/$id"
          params={{ authority, collection, id: item.record }}
          ...
          <div className="min-w-0">
            {t.authority && t.name ? (
              <Link
                ...
                params={{ authority: t.authority, collection: t.name }}
                className="underline-offset-4 hover:underline"
              >
                {name}
              </Link>
 ...
 * record opens in Data at its own kind, which needs that kind's collection from the
...
function ShippedRecordName({
  ...
  return (
    <Link
      to="/data/$authority/$collection/$id"
      params={{ authority: k.authority, collection: k.name, id: row.id }}
      className="block min-w-0 underline-offset-4 hover:underline"
    >
      {name}
    </Link>
